...
open class DPPickerManager: NSObject, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @objc open func showPicker(title: String?, view: UIView, completion:DPPickerCompletion?) {
        ...
        let image = UIImage.getFrom(customClass: DPPickerManager.self, nameResource: "close")?.withRenderingMode(.alwaysTemplate)
        ...
        case .landscapeLeft, .landscapeRight:
            ...
            buttonX = alertView.view.frame.size.height - ((image?.size.height ?? 0) * 2)
        case .portrait, .portraitUpsideDown:
            center = UIViewController.top?.view.center.x
            buttonX = alertView.view.frame.size.width - ((image?.size.width ?? 0) * 2)
        default: break
        }
        ...
        // close button
        let close = UIButton(frame: CGRect(x: buttonX - 10 , y: 10, width: (image?.size.width ?? 0), height: (image?.size.height ?? 0)))
